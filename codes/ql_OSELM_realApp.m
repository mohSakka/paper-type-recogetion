...
for chunk = 1:numberOfChunks
    for f = 1:numberOfFeatures
        ...
        if currentInd == 1
            ...
            [Learner{f}] = buildOSELMLearner(initData,Fn,trainFeatures,trainLabels,0.25,...
    ...
    newX = Features{bestAction(chunk,:)}(currentInd + numberOfTrain: currentInd + T -1,:);
    newY = Labels{bestAction(chunk,:)}(currentInd + numberOfTrain: currentInd + T -1);
...
save([cd '/../results/QL_OSELM_dataSetA.mat']);